//项目采用H5的localStorage存储数据（浏览器需IE8以上），如需实现向下兼容，则可以使用store.js来存储
//该项目考虑到TODO可能需要实现浏览器关闭后下次打开仍需要载入所需的TODO-list（假设应用场景：今天设定下周参加婚礼，浏览器关闭后重新打开TODO-list不能消失），所以使用localStorage而不使用sessionStorage
//保存
...
// 	1.1 添加任务，按钮动效 done
// 	1.2 确认添加按钮 done
// 	1.3 input输入监听，验证有无输入 done
// 4. 任务列表项 清除/完成 点击操作 done
// 5. 本地存储 添加/清除/完成 done
// 6. 任务列表渲染器 done

//添加任务按钮模块
(function() {
  var btnSVG = $("#addBtn svg") /* 添加按钮 */,
    addTaskContent = $("#addContent") /* 添加任务内容的标签页 */,
    okAddBtn = $(".okAddBtn") /* 确认添加任务 */,
    input = $("#value"); /* 添加任务内容 */
  /* 添加新的任务 */
  $("#addBtn").click(function(event) {
    if (addTaskContent.css("display") === "none") {
      addTaskContent.show("100"); /* display变成block */
      input.focus();
      /* 按钮旋转 */
      btnSVG.css({
      ...
      //弹出子按钮
      okAddBtn.css("bottom", "90px"); /* 相对于父元素向上浮动90px */
    } else {
  ...
  okAddBtn.click(function(event) {
    var text = input.val(); /* 获取input里面的内容 */
    if (text) {
      model.addTask(text);
      input.val("");
      addTaskContent.fadeOut("100"); /* 淡出 */
      btnSVG.css({
        transform: "rotate(0deg)",
        "-webkit-transform": "rotate(0deg)"
      });
...
// localStorage.removeItem('Jenson_completed');
// console.log(localStorage.getItem('Jenson_completed'));

//本地数据存储
function Model() {
  var addTask = function(s) {
    var taskItem = [localStorage["Jenson_todo"]];
    taskItem.push(s);
    localStorage.setItem(
      "Jenson_todo",
      taskItem
    ); /* 使用localStorage的setItem方法存储数据 */
    render.init();
  };
  var completeTask = function(t) {
    var arr = [localStorage["Jenson_completed"]];
    arr.push(t);
    localStorage.setItem("Jenson_completed", arr);
    var newArr = localStorage["Jenson_completed"].split(
      ","
    ); /* 方面后面求长度 */
    $("#completed-list").prepend(
      /* 在completed-list的前头加上语句和符号 */
      '<li class="task-item"><span class="task-title">' +
        t +
        "</span><i>" +
        (newArr.length - 1) +
  ...
  var clearTask = function(list, i) {
    var arr, listName, el;
    if (list == "todo") {
      listName = "Jenson_todo";
      el = "#task-list";
    } else if (list == "completed") {
      listName = "Jenson_completed";
      el = "#completed-list";
    }
    arr = localStorage[listName].split(",");
    arr.splice(i, 1); /* 删除第i个 */
...
//换肤功能 主题颜色控制器
function appColor() {
  var mainColor = localStorage.Jenson_color || "#25b99a",
    secColor = "#FFC107";
...
function colorPanel() {
  var html =
    "<ul class=setColorPanel><li class=colorItem data=#25b99a><span>青草绿</span><div class=colorBlock><div class=cgreen></div><div class=cgwhite></div></div><li class=colorItem data=#F44336><span>火热红</span><div class=colorBlock><div class=cred></div><div class=crwhite></div></div><li class=colorItem data=#2196F3><span>青花蓝</span><div class=colorBlock><div class=cblue></div><div class=cbwhite></div></div></ul>";
  $(".menu-subCard").html(html); /* 插入三种颜色 */
  $(".colorItem").click(function(event) {
    mainColor = $(this).attr("data");
    //颜色 本地存储
    localStorage.Jenson_color = mainColor;
    appColor();
    //动效
...
function aboutPanel() {
  var html =
    '<div class="aboutApp"><p><b>Jenson的TODO清单</b><br />"Version0.1,持续更新中..."<br /><br />"@2020"<a href="https://github.com/mimosa0837" target="_blank"><br />王佳圣</a></p></div>';
...
(function() {
  ...
  var sctA = $(document).scrollTop() /* 获取垂直坐标 */,
    headerHeight = $("#header").height();
